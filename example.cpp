...
DailyTimer timer1( 14, 37, 14, 39, EVERY_DAY, FIXED, customSeedGenerator);   // optional callback function for random number generation, see below example
DailyTimer timer2( 14, 38, 20, 30, SATURDAY, RANDOM_START);                   // default is FIXED, this will randomize the start time only
DailyTimer timer3( 14, 45,  1, 30, WEEKENDS);                                 // creates with a fixed start time and end time
bool timer1_LastState = false;

void setup() 
{
  setTime(1461422271);                        // set your sync provider!!
  Serial.begin(9600);
  pinMode(13, OUTPUT);
  ...
  //timer2.setRandomOffset(30, RANDOM_START); // Change to random start time, +/- 30 minutes... max 59 mins
